...
public class RawToProfileConverter extends Executor {
    ...
    /**
     * This method might be deleted or edited to be the new procedure. Work
     * in progress atm with Adam
     * @param params
     * @param infile
     * @param outfile
     * @param keepSam
    ...
    public String procedure(
            String[] parameters, String inFolder,
        ...
        if (fileDir.exists()) {
            ...
            try {
                ErrorLogger.log(
                        "SYSTEM",
                        "Files to be moved are: [" + filesToBeMoved + "]");
                ErrorLogger.log(
                        "SYSTEM",
                        "The path to move these files is: [" + outFilePath +
                        "]");
...
//			cleanUp(toBeRemoved);
    ...
    private File[] initiateProcedure(
            ...
            String inFolder,
            String outFilePath) throws ProcessException {
    /*If you want to run several processes simultaneously, this would need to
     be changed*/
    ...
    private boolean validateParameters(String[] parameters)
        ...
        if (checker.shouldRunRatioCalc()
            && !validator.validateRatioCalculation(
                parameters[6],
                parameters[7])) {
            isOk = false;
        }
    ...
    private void runSmoothing(String[] parameters, boolean isRatioCalc)
        ...
        if (isRatioCalc) {
            ...
            filesToSmooth = new File(sortedDirForFile + "reads_gff/allnucs_sgr")
                    .listFiles();
            dirToFiles = new File(
                    sortedDirForFile + "reads_gff/allnucs_sgr/smoothed");
    ...
    private void doRatioCalculation(String dirPath, String[] parameters)
        ...
        try {
            ...
            throw new ProcessException(
                    "Process interrupted while running ratio calculation on " +
					"files in folder "
                    + dirPath);
        } catch (IOException e) {
            throw new ProcessException(
                    "Could not read or write to files while running ratio " +
					"calculation in folder: "
                    + dirPath);
        }
    }
    ...
    private String runBowTie(String fileOne, String fileOneName)
        ...
        try {
            ...
            throw new ProcessException(
                    "Could not run bowTie on file: "
                    + fileOneName +
                    ", please check your input and permissions");
        }
    }
    ...
    private void sortSamFile(String unsortedSamFileName)
        ...
        try {
            ...
            throw new ProcessException(
                    "Process interrupted by external "
