...
public class AddFileToExperimentCommand extends Command {
	...
	public boolean validate() throws ValidateException {
		...
		if(uploader != null){
			if(uploader.length() > MaxSize.FILE_UPLOADER ||
					uploader.length() < 1) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Uploader name: " + uploader + " has to be between 1 " +
						"and " + database.constants.MaxSize.FILE_UPLOADER +
								" characters long.");
			}
			if(!hasOnlyValidCharacters(uploader)) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Invalid characters in uploader name. Valid " +
		...
		if(metaData != null){
			if(metaData.length() > MaxSize.FILE_METADATA ||
					metaData.length() < 1) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Metadata has to be between 1 and " +
						database.constants.MaxSize.FILE_GRVERSION +
								" characters long.");
			}
			if(metaData.contains("/")) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Metadata can not contain slash.");
			}
		}
		if(author != null){
			if(author.length() > MaxSize.FILE_AUTHOR || author.length() < 1) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Author has to be between 1 and " +
						database.constants.MaxSize.FILE_AUTHOR +
								" characters long.");
			}
			if(!hasOnlyValidCharacters(author)) {
				throw new ValidateException(StatusCode.BAD_REQUEST,
						"Invalid characters in author name. Valid characters" +
								" are: " + validCharacters);
			}
		}
		...
		if(fileName.contains("/")) {
			throw new ValidateException(StatusCode.BAD_REQUEST,
					"File name can not contain slash.");
		}
