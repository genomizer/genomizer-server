...
public class SecureDoorman {
	...
	/**
	 * Constructor. Creates a HttpsServer (but doesn't start it) which listens on the given port.
	...
	* SSL context and parameters.*/
	private HttpsConfigurator getHttpsConfiguration(String password){
		...
		try {
			...
			keyStore.load(new FileInputStream(filename),charPassword);
		...
		//Creates the configurator object for the server
		HttpsConfigurator httpsConfig = new HttpsConfigurator(sslContext){
			...
			public void configure (HttpsParameters params) {
				...
				try {
					//Initialize the ssl context and parameters
	...
	/**
	 * Handles the request information from the client and creates a
	...
	private void handleRequest(HttpExchange exchange, CommandType type) {
		...
		//Decodes the URL and creates a new command from that.
