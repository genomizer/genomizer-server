...
Pyicos default experiment and output format is a derivative of UCSC `Bed format <http://genome.ucsc.edu/FAQ/FAQformat.html#format1>`_. It follows the same 
starting fields "chromosome/tag start end" but it uses some of the original optional fields to include extra information. It is a cluster oriented format that aims to recollect information of a cluster of reads in a single comprehensive line. 
...
.. 11) Score_height: (Height + -log10(p-value_height))/2 ; can be used to rank the peaks

.. 12) Score_area: (Area + -log10(p-value_area))/2 ; can be used to rank the peaks 
